...
import styles from './Integrators.module.css';
...
export const IntegratorsModule = () => {
  ...
  useEffect(() => {
    if (users?.length) {
      setCurrentUsers(
        users.filter((user) => user.id !== userId).filter((user) => user.group_id === '2'),
      );
    }
  }, [users, role, userId]);
